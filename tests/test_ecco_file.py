...
def test_ecco_day_snap_mdsfilestr():
    """Test MDS filestr parsing and reconstruction for day_snap."""
    # "circularity" test:
    ef = ecco_dataset_production.ecco_file.ECCOMDSFilestr(
        filestr=ECCO_DAY_SNAP_RESULTS_FILE)
    assert ECCO_DAY_SNAP_RESULTS_FILE == \
...
def test_ecco_day_mean_mdsfilestr():
    """Test MDS filestr parsing and reconstruction for day_mean."""
    # "circularity" test:
    ef = ecco_dataset_production.ecco_file.ECCOMDSFilestr(
        filestr=ECCO_DAY_MEAN_RESULTS_FILE)
    assert ECCO_DAY_MEAN_RESULTS_FILE == \
        ecco_dataset_production.ecco_file.ECCOMDSFilestr(
            prefix=ef.prefix,
            averaging_period=ef.averaging_period,
            time=ef.time,
...
def test_ecco_day_snap_granulefilestr():
    """Test granule filestr parsing and reconstruction for day_snap."""
    # "circularity" test:
    ...
    assert ECCO_DAY_SNAP_GRANULE_FILE == \
...
def test_ecco_day_mean_granulefilestr():
    """Test granule filestr parsing and reconstruction for day_mean."""
    # "circularity" test:
    epf = ecco_dataset_production.ecco_file.ECCOGranuleFilestr(
        filestr=ECCO_DAY_MEAN_GRANULE_FILE)
    assert ECCO_DAY_MEAN_GRANULE_FILE == \
        ecco_dataset_production.ecco_file.ECCOGranuleFilestr(
            prefix=epf.prefix,
            averaging_period=epf.averaging_period,
            date=epf.date,
