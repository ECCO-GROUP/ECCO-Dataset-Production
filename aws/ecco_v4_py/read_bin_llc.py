...
def read_llc_to_compact(fpath, llc=90, skip=0, nk=1, nl=1,
    ...
    ----------
    fpath : string
        A string with the path to the binary file to open
    ...
    """
    data_compact = load_binary_array(fpath, llc, 13*llc, nk=nk, nl=nl,
...
def read_llc_to_tiles(fpath, llc=90, skip=0, nk=1, nl=1,
    ...
    ----------
    fpath : string
        A string of the path to the binary file to open
    llc : int
        the size of the llc grid.  For ECCO v4, we use the llc90 domain
        so `llc` would be `90`.
        Default: 90
    ...
    if use_xmitgcm:
        ...
        data_compact = read_llc_to_compact(fpath, llc=llc, skip=skip, nk=nk, nl=nl,
