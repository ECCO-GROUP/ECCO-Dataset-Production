...
def write_attributes_tables_tex(base_dir: str, config_dict_static: dict, config_dict_user: dict) -> None:
    """
    Generate and write LaTeX longtable files for each unique attribute type in the config.

    Iterates over the config dict to identify all attribute types (keys
    ...
    .. note::
        The ``latex_lines`` list retrieved from ``config_dict`` is copied
    ...
    :type base_dir: str
    :param config_dict: Configuration mapping containing attribute metadata.
    ...
    for key in config_dict_static.keys():
        if "_attributes_" in key:
            ...
            if attribute_type not in processed_attribute_types:
                ...
                for suffix in required:
                    if f"{attribute_type}_attributes_{suffix}" not in config_dict_static:
                ...
                #latex_lines = write_table(base_dir, config_dict, attribute_type)
                if attribute_type == "global":
                    write_table(base_dir, config_dict_static, config_dict_user, attribute_type, global_attributes_granules)
                else:
                    write_table(base_dir, config_dict_static, config_dict_user, attribute_type, non_global_attributes_granules)

                '''
                latex_output_file = os.path.join(base_dir, config_dict[f"{attribute_type}_attributes_tex_file"])
...
def obtain_json_data(base_dir: str, filename: str) -> list:
    ...
    with open(os.path.join(base_dir, filename), "r") as file:
        json_data_dict_list = json.load(file)
    return json_data_dict_list


def modify_json_add_introduction_field_to_groupings(json_data: list, filename: str, config_dict_static: dict, config_dict_user: dict) -> list:
    ...
    for dict in json_data:
        ...
        intro_string = f"{config_dict_static['dataset_text_dict']['opening_text']}{modified_dict['name']}"
        frequency_list_crude = [f.strip() for f in modified_dict['frequency'].split(',')]
        frequency_list_verbose = []

        for frequency_crude in config_dict_static['dataset_text_dict']['frequency_dict'].keys():
            if frequency_crude in frequency_list_crude:
                frequency_list_verbose.append(config_dict_static['dataset_text_dict']['frequency_dict'][frequency_crude])
        
        if len(frequency_list_verbose) == 0:
            intro_string = f"{intro_string}{config_dict_static['dataset_text_dict']['time_resolution_none_text']}"

        elif len(frequency_list_verbose) == 1:
            intro_string = f"{intro_string}{config_dict_static['dataset_text_dict']['time_resolution_single_text']}"
            intro_string = intro_string.format(frequency_string = frequency_list_verbose[0])

        elif len(frequency_list_verbose) == 2:
            intro_string = f"{intro_string}{config_dict_static['dataset_text_dict']['time_resolution_multiple_text']}"
        ...
        for grid_type in [g.replace("-","") for g in config_dict_user['grid_types_considered']]: 
            if grid_type.casefold() in filename.casefold():
                intro_string = f"{intro_string}{config_dict_static['dataset_text_dict'][f'grid_text_{grid_type}']}"
                intro_string = intro_string.format_map(format_map_context_dict)
                #intro_string.format_map(format_map_context_dict)
                #pdb.set_trace()
                break
...
def obtain_keys(json_data: list) -> set:
    ...
    :returns: Union of all keys found across every dict in the list.
    :rtype: set[str]
    """
    keys = set()
...
def write_table(base_dir: str, config_dict_static: dict, config_dict_user: dict, attribute_type: str, attributes_granules: set) -> list:
    """
    ...
    :param dict_list_from_json: Records loaded from a JSON file, each
        representing one table row. Keys define the columns.
    :type dict_list_from_json: list[dict]
    :param config_dict: Configuration mapping passed through to the
    ...
    latex_lines = list(config_dict_static[f"{attribute_type}_attributes_latex_lines"])
    dict_list_from_json = obtain_json_data(base_dir, config_dict_static[f"{attribute_type}_attributes_json_file"].format(ecco_version_string=config_dict_user['ecco_version_string']))
    ...
    # the first field inserted into the dict, ie is the first field listed in the json file.  This logic will break
    # if that is ever not the case.
    attributes_list_from_json = []
    for dict in dict_list_from_json:
        attributes_list_from_json.append(list(dict.values())[0].lower()) 
        #pdb.set_trace()
    ...
    for attribute in sorted(list(attributes_granules)):
        if attribute.lower() not in attributes_list_from_json:
            ...
            #raise KeyError(f"Attribute ‘{attribute}’ from your downloaded granules is not present in the list of approved attributes (see {config_dict[f'{attribute_type}_attributes_json_file']})")

            mystery_attributes.append(attribute)

        else:
            dict_index = attributes_list_from_json.index(attribute)
            formatted_dict_as_list = [
                utils_general.sanitize_with_url(config_dict_static, str(dict_list_from_json[dict_index].get(key, "N/A")))
                for key in dict
            ]

            # Update the max column count seen so far
            max_col = len(formatted_dict_as_list) if len(formatted_dict_as_list) > max_col else max_col

            # Pad with empty cells if this row has fewer columns than the widest row
            if len(formatted_dict_as_list) < max_col:
                formatted_dict_as_list.extend([""] * (max_col - len(formatted_dict_as_list)))

            latex_lines.append(
                r'\rowcolor{' + config_dict_static[f"{attribute_type}_attribute_rowcolor"] + '} ' + '\n' + ' & '.join(formatted_dict_as_list) + r' \\ \hline' + '\n'
            )

    latex_lines.append(r'\end{longtable}')

    latex_output_file = Path(base_dir) / config_dict_static[f"{attribute_type}_attributes_tex_file"].format(ecco_version_string=config_dict_user['ecco_version_string'])
    #latex_output_file = os.path.join(base_dir, config_dict_static[f"{attribute_type}_attributes_tex_file"].format(config_dict_user['ecco_version_string']))
    # Create any missing parent directories for the output path
    #Path(latex_output_file).parent.mkdir(parents=True, exist_ok=True)
    ...
    print() 
    print(f"For a list of allowed {attribute_type} attributes, see ‘{config_dict_static[f'{attribute_type}_attributes_json_file'].format(ecco_version_string=config_dict_user['ecco_version_string'])}’.")
    print() 
    print() 
    print() 
...
def check_for_attributes(base_dir: str, config_dict_static: dict, config_dict_user: dict, required_ratio: float) -> None:
    ...
    granules_attributes_dict = {}
    granules_attributes_dict['global'] = {}
    granules_attributes_dict['variable'] = {}

    for granule_path in all_granule_paths:
        dataset = xr.open_dataset(granule_path)
        granules_attributes_dict['global'][dataset.attrs['product_name']] = [s.strip() for s in list(dataset.attrs.keys())]
        ...
        granules_attributes_dict['variable'][dataset.attrs['product_name']] = temp_dict
        #granules_attributes_dict['variable'][dataset.attrs['product_name']] = variable_attributes_temp_dict
    ...
    for global_attributes_list in granules_attributes_dict['global'].values():
    ...
    for temp_dict in granules_attributes_dict['variable'].values():
    ...
    for attribute_type in ['global', 'variable']:
        ...
        attributes_list_from_json = []
        for dict in dict_list_from_json:
            attributes_list_from_json.append(list(dict.values())[0]) 
        ...
        if attribute_type == 'global':
            ...
            for granule_name in granules_attributes_dict[attribute_type].keys():
                new_attributes_flags = [True] * len(granules_attributes_dict[attribute_type][granule_name])
                for ii in range(len(granules_attributes_dict[attribute_type][granule_name])):
                    if granules_attributes_dict[attribute_type][granule_name][ii] in attributes_list_from_json:
                        new_attributes_flags[ii] = False
                    new_attrs_granule = set([attr for attr,flag in zip(granules_attributes_dict[attribute_type][granule_name], new_attributes_flags) if flag])
            ...
            for var_type in var_types_to_consider:
                ...
                for granule_name in granules_attributes_dict[attribute_type].keys():
                    ...
                    #for ii in range(len(granules_attributes_dict[attribute_type][granule_name][var_type])):
                    for attr_list in granules_attributes_dict[attribute_type][granule_name][var_type].values():
                        new_attributes_flags = [True] * len(attr_list)
                        for ii in range(len(attr_list)):
                        #if granules_attributes_dict[attribute_type][granule_name][var_type][ii] in attributes_list_from_json:
            ...
            for granule_name in granules_attributes_dict[attribute_type].keys():
                ...
                if attribute_type == 'global':
                    ...
                    for attribute in sorted(granules_attributes_dict[attribute_type][granule_name]):
                    ...
                    for attribute in sorted(attributes_list_from_json):
                        if attribute in attributes_from_json_absent_from_all:
                            continue
                        if attribute not in granules_attributes_dict[attribute_type][granule_name]:
                    ...
                    for var_type in var_types_to_consider:
                        ...
                        temp_dict = granules_attributes_dict[attribute_type][granule_name][var_type]
