...
def fieldTable(config_dictionary: dict, dataset: xr.Dataset, is_coord: bool, grid_type: str) -> list:
    ...
    latex_lines.append(fr"\caption{{Coordinates and variables from {utils.sanitize(config_dictionary, product_name)} ({grid_type})}}")
    ...
    for ij in np.arange(len(coordvar_shortname)):
        latex_lines.append(
            f'{utils.sanitize(config_dictionary, coordvar_shortname[ij])} &'
            f'{utils.sanitize(config_dictionary, coordvar_longname[ij])} &'
            rf'{utils.sanitize(config_dictionary, coordvar_units[ij])}  \\ \hline'
        )
    ...
    for ij in np.arange(len(datavar_shortname)):
        latex_lines.append(
            f'{utils.sanitize(config_dictionary, datavar_shortname[ij])} &'
            f'{utils.sanitize(config_dictionary, datavar_longname[ij])} &'
            rf'{utils.sanitize(config_dictionary, datavar_units[ij])}  \\ \hline'
        )
...
def latex_example_netcdf(base_dir: str, config_dictionary: dict, grid_type: str) -> None:
    ...
    for dimension_name in dataset.sizes:
        ...
        latex_lines = utils.append_hanging_indentation_commands_cm_latex(config_dictionary, num_tabs, latex_lines)
        latex_lines.append(
            utils.sanitize_with_math(config_dictionary, f'{dimension_name} = {len(dataset[dimension_name])}') + "\\\\"
        )
...
def data_var_table(
    ...
    :rtype: list[str]
    """
    dataset_name_formatted = utils.sanitize(config_dictionary, dataset_name)
    ...
    cdl_description = utils.sanitize_with_math(config_dictionary, attrs['CDL Description'])
    ...
    la = [
        ...
        fr"\caption{{{utils.sanitize(config_dictionary, field_name)} from {dataset_name_formatted} ({grid_type})}}"
...
def global_attrs_for_ECCOnetCDF(
    ...
    for i in list(GlobAttrsFilledECCO.keys()):
        ...
        latex_lines.append(
            rf'{utils.sanitize(config_dictionary, GAttrsNam)} & {GAFormat} & '
            rf'{utils.sanitize(config_dictionary, GAdescription)} & {GASource} \\ \hline'
        )
...
def data_products(
    ...
    for json_dictionary in list_of_json_dictionaries:
        ...
        data_array_list, dataset = search_and_extract(
            ...
            os.path.join(base_dir, granule_directory),
            is_coord
        )
