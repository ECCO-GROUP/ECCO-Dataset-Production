...
Applies ECCO metadata (global, variable, and coordinate attributes) to existing
NetCDF files without requiring external grid files or mapping factors.

Overview
--------

``edp_add_metadata`` is designed for "bare" NetCDF files (like grid geometry files)
that already contain their coordinate data but need standardized ECCO metadata added.
This tool does NOT load external grid files or mapping factors - it works with the
coordinates already present in the input file.

**Key Differences from Other Tools:**

- ``edp_add_metadata``: For bare NetCDF files with coordinates. No external grid/factors needed.
- ``process_time_invariant_granule()``: Requires grid files and/or mapping factors to add coordinate bounds.
- ``edp_generate_datasets``: Full pipeline that loads MDS binary files and creates granules from scratch.
...
.. code-block:: bash
    ...
    edp_add_metadata -i INPUT -o OUTPUT -m METADATA -c CONFIG
                     [-g {native,latlon}] [-d {2D,3D}] [-s]
                     [-l LOG_LEVEL] [--keygen KEYGEN] [--profile PROFILE]
...
``-o, --output``
    Path for output NetCDF file with metadata applied.

``-m, --metadata``
    Path to ECCO metadata directory (or S3 bucket/prefix).

``-c, --config``
    Path to ECCO configuration YAML file (or S3 object).

Optional Arguments
^^^^^^^^^^^^^^^^^^

``-g, --grid-type``
    Grid type: ``native`` or ``latlon``.
    Default: ``native``

``-d, --dimension``
    Dataset dimension: ``2D`` or ``3D``.
    Default: Auto-detect from file (checks for vertical dimensions: Z, k, k_l, k_u, k_p1).
    If any vertical dimension exists, dataset is 3D; otherwise 2D.

``-s, --strip-attributes``
    Strip all existing attributes before applying new ones.
    Default: False (preserves existing attributes)
...
``--keygen``
    AWS SSO key generation script path (for S3 access).

``--profile``
    AWS profile name (e.g., ``saml-pub``, ``default``).


Use Cases
---------

1. **Grid Geometry Files**
   Adding metadata to grid geometry NetCDF files created by external tools

2. **Pre-processed Data**
   Adding ECCO-compliant metadata to NetCDF files from other sources

3. **Metadata Updates**
   Updating metadata on existing files while preserving data

4. **Bootstrapping**
   Creating initial grid files that will later be used by other tools


Entry Point
-----------

**Module:** ``ecco_dataset_production.apps.add_metadata``
...
**Basic example (dimension auto-detected):**
...
The tool applies comprehensive ECCO-compliant metadata to your NetCDF files:
...
- Boolean and integer types (uint8, int8, uint16, int16, int32, int64) are preserved
  with their original precision
- Only floating-point types are converted to the configured precision (default: float32)
...
- Creation timestamps (``date_created``, ``date_modified``)
- GCMD keywords
- UUID
...
When the output filename contains ``GRID_GEOMETRY``, the tool automatically uses
dataset-specific descriptions from ``grid_geometry_dataset_descriptions.json`` in
...
.. code-block:: text
    ...
    This dataset provides geometric parameters for the lat-lon-cap 90 (llc90)
    native model grid from the ECCO Version 4, Release 6 ocean and sea-ice
    state estimate. Parameters include areas and lengths of grid cell sides;
    horizontal and vertical coordinates of grid cell centers and corners;
    grid rotation angles; and global domain geometry including bathymetry
    and land/ocean masks.

**Lat-Lon Grid Geometry Example:**

.. code-block:: text

    This dataset provides geometric parameters for the regular 0.5-degree
    lat-lon grid from the ECCO Version 4, Release 6 (V4r6) ocean and sea-ice
    state estimate. [...] However, users should note these domain geometry
    fields are approximations because they have been interpolated from the
    ECCO lat-lon-cap 90 (llc90) native model grid.
...
The tool automatically detects whether your dataset is 2D or 3D by inspecting
NetCDF dimensions:
...
You can override auto-detection by explicitly specifying ``--dimension 2D`` or
``--dimension 3D``.
...
2. **Have recognizable variable names**: Variable names should match those in metadata JSON files
...
Variable names in your NetCDF must match those defined in ``*variable_metadata*.json``.
Check variable names with:
